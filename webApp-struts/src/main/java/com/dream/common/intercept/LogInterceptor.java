...
import com.dream.common.utils.log.LogStack;
...
/**
 * Created by chenrui on 15/12/11.
 */
public class LogInterceptor extends AbstractInterceptor {
    ...
    public String intercept(ActionInvocation invocation) throws Exception {
        ...
        request.setAttribute(LogUtils.DEBUG_LOG, LogUtils.initLog(method + ' ' + version));
        return invocation.invoke();
    }
     ...
     * @param response 结构化response
     */
    public static void setDebugMsg(HttpServletRequest request, Object response) {
        if(request == null || response == null || !(response instanceof RopResponse)) {
            return;
        }
        if(isDebug(request)) {
